...
class LogIn {
    ...
    function emailUser($ToMailStr, $NameStr, $PinStr) {
        ...
        $HeadersArr = array('Authorization: Bearer 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', 'Content-Type: application/json');
    ...
    function validatePin($PinStr) { // validate OTP when user types it onto swal
        ...
        if ($PrepObj->num_rows > 0) {
            ...
            if (password_verify($PinStr, $HashedPinStr)) {
                ...
                $ResultObj = $this->ConnectionObj->query($SelectUserStr);
                $RowArr = $ResultObj->fetch_assoc();
                $this->createSession($RowArr['User'], $RowArr['EmailAddress'], $RowArr['FirstName'], $RowArr['LastName'], $RowArr['ID'], $PinStr, $RowArr['Userpfp']);
    ...
    function deletePost($IdInt) {
        ...
        $SelectJoinStr = "SELECT post.ID post.UserId FROM post LEFT JOIN user ON post.UserId = user.ID WHERE post.ID=?";
        $PrepObj = $this->ConnectionObj->prepare($SelectJoinStr);
        $PrepObj->bind_param("i", $IdInt);
        $PrepObj->execute();
        $PrepObj->store_result();
        ...
        if ($PrepObj->num_rows > 0) {
            // found
            $PrepObj->bind_result($PostIdInt, $PostUserIdInt);
            $PrepObj->fetch();
            if ($PostUserIdInt === $_SESSION['id']) { // check if user is deleting their own post
                $DeleteRowStr = "DELETE FROM post WHERE ID=$PostIdInt";
                if ($this->ConnectionObj->query($DeleteRowStr)) {
                    // row deleted successful
                    $ReturnStr = "success";
                } else {
                    // error occured when deleting
                    $ReturnStr = "fail";
                }
            } else {
                // this user cannot delete someone else's post
                $ReturnStr = "deleteErr";
            }
        } else {
            // not found
            $ReturnStr = "no results found";
        }
    ...
    function editPost($IdInt, $ImgStr, $TitleStr, $TextStr, $UpdateTimeStampStr) {
        // edited post syubmitted here
        $ReturnStr = "";
        $PostIdInt = null;
        $PostUserIdInt = null;
        $this->createConn();
        ...
        if ($PrepObj->num_rows > 0) {
            ...
            $PrepObj->fetch();
            if ($PostUserIdInt === $_SESSION['id']) { // check if user is deleting their own post
                $DeleteRowStr = "UPDATE post SET  WHERE ID=$PostIdInt";
                if ($this->ConnectionObj->query($DeleteRowStr)) {
                    // row deleted successful
                    $ReturnStr = "success";
                } else {
                    // error occured whe deleting
                    $ReturnStr = "fail";
                }
            } else {
                // this user cannot delete someone else's post
