...
function createDeleteBtn(postId) {
    // create delete button
    const deleteBtn = document.createElement('button');
    deleteBtn.classList.add('btn', 'btn-danger', 'rounded-pill', 'btn-more-icon', 'float-end');
    deleteBtn.setAttribute('type', 'button');
    deleteBtn.setAttribute('id', postId);
    deleteBtn.setAttribute('onclick', 'deletePost(this.id)');
    const icon2 = document.createElement('i');
    icon2.className = "material-symbols-outlined";
    const text2 = document.createTextNode("delete");
    icon2.appendChild(text2);
...
function createEditBtn(postId) { // pending
    // create edit button
    const editBtn = document.createElement('button');
    editBtn.classList.add('btn', 'btn-primary', 'rounded-pill', 'btn-more-icon', 'float-end');
    editBtn.setAttribute('type', 'button');
    editBtn.setAttribute('id', postId);
    editBtn.setAttribute('onclick', 'editPost(this.id)'); // change this
    const icon = document.createElement('i');
    icon.className = "material-symbols-outlined";
    const text1 = document.createTextNode("edit");
    icon.appendChild(text1);
...
function createTableFromObjects(data) {
    ...
    if (Array.isArray(data) && data.length) {
        for (obj of data) {
            ...
            // create a delete and edit button
            // editBtn = createEditBtn(obj[6].toString()); // NEW
            // deleteBtn = createDeleteBtn(obj[6].toString()); // NEW
            // dataCell.appendChild(editBtn); // NEW 
            // dataCell.appendChild(deleteBtn); // NEW 
...
function completeRecaptcha() { // login
    Swal.fire({
...
function pinSignUp() { // verify user sign up (not in use??)
...
function deletePost(postId) { // home
    const swalWithBootstrapButtons = Swal.mixin({
        customClass: {
            confirmButton: 'btn btn-primary',
            cancelButton: 'btn btn-danger'
    ...
    reverseButtons: false
        ...
        if (result.isConfirmed) {
            const postParameters =  new URLSearchParams();
            postParameters.append("function", "deletePost");
            postParameters.append("postId", parseInt(postId));
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    if (newResult === "success") {
                        successPopup("Your post has been deleted.");
                    } else {
                        errPopup(newResult);
                    }
                    // console.log(newResult);
...
function saveEditChanges() { // home
    // save the edited changes
}

// click delete >>> popup->"are you sure" >>> delete that post
// ...          >>> takes id of card      >>> requests delete for that id