<?php
session_start();
// header("Content-type: image/jpeg"); // no changes are observed
...
if (isset($_POST["function"])) {
    ...
    switch($_POST["function"]) {
        ...
        case "getProfilePicture":
            // issues remain with display of profile picture
            $SOURCECONSTANTSTR = "data:image/jpeg;base64,";
            // $ResultStr = $UserObj->returnProfilePicture($_SESSION['id']);
            $ProfilePictureStr = $SOURCECONSTANTSTR . $_SESSION['pfp'];
            // error_log(json_encode($ProfilePictureStr));
        ...
        break;
        case "editPost":
            $CurrentTimeStampStr = date('Y-m-d H:i:s'); // get timestamp when post was edited
            $ResultObj = $UserObj->editPost($_POST['postId'], $_POST['img'], $_POST['Title'], $_POST['Text'], $CurrentTimeStampStr);
            echo json_encode($ResultObj);
        break;
        case "deletePost":
            $ResultObj = $UserObj->deletePost($_POST['postId']);
            echo json_encode($ResultObj);
        break;
    }
}