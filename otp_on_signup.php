...
class SignUp {
    ...
    function usePin($PinStr) {
        ...
        if ($this->ifBlock()) {
            ...
            $SelectPinStr =  "SELECT Pin, Used FROM signup WHERE EmailAddress=?";
            ...
            if ($PrepObj->num_rows > 0) { // check if exists
                ...
                if ($UsedInt < 3) { // check if used
                    if (password_verify($PinStr, $PinDBStr)) { // check if pin is correct
                        $ReturnStr = "Successful";
                    } else {
                        $ReturnStr = "wrong pin";
                        $EmailStr = $_SESSION['Email'];
                        $this->ConnectionObj->query("UPDATE signup SET Used=Used+1 WHERE EmailAddress='".$EmailStr."'"); // UPDATE MyGuests SET lastname='Doe' WHERE id=2
                    }
                } else {
                    $ReturnStr = "Pin already used";
                    $IpStr = $_SERVER['REMOTE_ADDR'];
                    $this->ConnectionObj->query("INSERT INTO blocked (IP) VALUES ('".$IpStr."')");
                }
            } else {
